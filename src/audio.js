/* =========================================================================
   ...
   Music playback lives in music.js, which imports tone/noise/getCtx/getMusicBus
   from here. The `music` export is re-exported below so all other modules keep
   importing from "./audio.js" unchanged.
   ========================================================================= */
import { CFG } from "./config.js";
export { music } from "./music.js";
import { music as _music } from "./music.js";
...
// Accessors for music.js (ctx/musicBus are lazily created; can't export bindings).
...
export function unlock(){
  ...
  if (!_unlocked){
    ...
    if (!_music.isPlaying()){
      setTimeout(() => {
        if (!_music.isPlaying()) _music.playTitle();
      }, 50);
    }
  }
}
...
// bus defaults to sfxBus; music scheduler passes musicBus.
export function noise({ dur, gain = 0.5, filterType = "bandpass", filtFreq = 1200, filtEnd, Q = 1, delay = 0, bus }){
  const t0 = ctx.currentTime + delay;
