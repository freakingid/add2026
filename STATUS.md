...
- **`state.js`** — `G` (the mutable container: run meta + entities `dan/shots/enemies/terminals/pickups/marks/floats/ebolts/vending/workers/camera/exit` + `spawnTimer`/`pickupTimer`) and `levelType()`. ← config.
...
- **`enemies.js`** — `spawnEnemy` (per-type init), `updateEnemies` (dispatch + melee contact via `combat`), per-type AI `updatePicker/Forklift/Security/Sorter/Cleaner/Drone/Manager/Scanner/Inventory`, `buffSpd` (combined Manager-berserk + Scanner-alarm speed mult), Cleaner/Scanner patrol routing (`nearestWaypoint`/`buildCleanerPatrol`/`advancePatrol`) + Cleaner spray helpers (`danInSprayCone`/`coneRayDist`(exported, also clips the rendered cone)/`applySpray`). ← config, state, world, combat, projectiles, **workers** (`killWorker`, for the Inventory Bot).
- **`level.js`** — `newGame` (full reset) → `buildLevel` (world + terminals + exit + 5 workers + vending machines; single-type, or the `"mixed"` branch seeding one terminal of every real type; keeps HP/powerups/score) → `nextLevel`; spawner-terminal emission `spawnFromTerminal`/`spawnWave`; pickups `spawnPickup`/`updatePickups`. ← config, state, world, enemies, vending, effects.
...
- **`render.js`** — `render()` compositor + world/entity draws (`drawFloor`/`drawWalls`/`drawMarks`/`drawExit`/`drawExitPointer`/`drawVending`/`drawTerminals`/`drawShots`/`drawPickups`/`drawWorkers`/`drawFloats`/`drawDan`). ← canvas, state, config, palette, world, render-entities, screens.
...
sandbox. The **remaining larger GDD features** are the next valuable work:

- **Atomic Dustbin special (§5)** — the deployable that attracts robots (drones too, via `e.flying`) then detonates for big AoE + an exponential combo bonus. A pickup + a Dan-held item + an attract/detonate sequence; no enemy work, but touches input (§4.4 the F key — already in `HANDLED_KEYS`), pickups, and a new effect.
