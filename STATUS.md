...
- **`optionsmenu.js`** — shared Options + Controls screens: `openOptions`/`openControls`/`optionsScreen`/`defaultPane`/`handleOptionsEdge`/`drawOptions`. Owns the volume sliders + mute row (moved from `pause.js`) plus a 5th "CONTROLS ▸" row, and a Controls sub-screen (keyboard/gamepad pane toggle; pane art is a Phase 4 placeholder, real legend art lands Phases 5-6). `pause.js` delegates nav/draw here instead of owning options state directly. ← canvas, palette, state, audio (volume/mute getters+setters), savegame (`savePrefs`), screens (`drawFireLegend`, unused until the Phase 5-6 art pass). Must NOT import `input.js`/`pause.js` (they import it).
