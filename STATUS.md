...
## Next enemy — Inventory Bot (implementation notes)

Roadmap step 9, and the last enemy in the GDD roster. **Blocked:** it hunts human
workers (GDD §6.1.6 / §7), and human workers + rescue scoring don't exist yet —
build that feature first, then this bot.

- **Wanderer / worker-hunter:** HP 1, 75 pts, melee contact. Roams (not a Dan-chaser by default); when a human worker is in range it switches to hunting that worker. Needs the workers entity + spawning first.
- **Register checklist (same shape as the others):** `ENEMY.inventory`, `LEVEL_PLAN += "inventory"`, `TERMINAL_TINT.inventory`, `COL` entries, `spawnEnemy` init, `updateInventory`, dispatch in `updateEnemies`/`drawEnemies`, `drawInventory`, HUD `typeName`. No new projectile kind needed.

Until workers exist, the more valuable next steps are the **larger unbuilt GDD
features** below (workers + rescue scoring, the Atomic Dustbin special, audio).
